...
def get_caption_from_api(image_data):
    ...
    if response.status_code == 200:
        ...
        print(f"Erro ao processar imagem: {response.status_code}, {response.text}")
        return None
...
for filename in os.listdir(image_folder):
    if filename.endswith((".png", ".jpg", ".jpeg")):  # Filtra apenas arquivos de imagem
        ...
        if downscaled_image:
            print(f"Imagem {filename} redimensionada para 512x512 ou menor.")
            image_data = downscaled_image
        else:
            with open(image_path, "rb") as image_file:
                image_data = image_file.read()

        caption = get_caption_from_api(image_data)
        request_count += 1
